...
/*   Updated: 2019/04/04 00:40:24 by alamit           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				ft_conv_f(t_buff *buff, const char *format, t_float80 n)
{
	...
	num_digits = 0;
	if (b10.get_digits)
		num_digits = b10.get_digits(&b10, f.precision > -b10.exp ? b10.exp + f.precision : 0);
	prefix(buff, &f, &b10);
	if (b10.nan)
	...
	return (!ft_buffc(buff, '0', 1)
		|| ((f.precision || ft_strchr(f.flag, '#')) && !ft_buffc(buff, '.', 1))
		|| !ft_buffc(buff, '0', (f.precision > -b10.exp ? -b10.exp : f.precision))
		|| !ft_buff(buff, b10.buf, num_digits)
		|| !ft_buffc(buff, '0', (f.precision > -b10.exp ? b10.exp + f.precision : 0) - num_digits));
}
